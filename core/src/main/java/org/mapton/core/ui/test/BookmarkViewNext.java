 ...
 */
package org.mapton.core.ui.test;
...
import org.mapton.core.ui.grid.GridFileExportAction;
import org.mapton.core.ui.grid.GridFileImportAction;
 ...
 */
public class BookmarkViewNext extends BorderPane {

    private final Map<String, TreeItem<MBookmark>> mBookmarkParents = new TreeMap<>();
    private final ResourceBundle mBundle = NbBundle.getBundle(BookmarkViewNext.class);
    ...
    private void createUI() {
        ...
        ButtonBar toolBar = ActionUtils.createButtonBar(actions);
        toolBar.setButtonMinWidth(getIconSizeToolBarInt());
        FxHelper.adjustButtonWidth(toolBar.getButtons().stream(), getIconSizeToolBarInt());
        toolBar.getButtons().stream().filter((item) -> (item instanceof ButtonBase))
                .map((item) -> (ButtonBase) item).forEachOrdered((buttonBase) -> {
            System.out.println(buttonBase);
            System.out.println(buttonBase.prefWidthProperty());
            buttonBase.setPrefWidth(getIconSizeToolBarInt());
            System.out.println(buttonBase.prefWidthProperty());
            FxHelper.undecorateButton(buttonBase);
        });
//        BorderPane bp = new BorderPane(mFilterTextField);
//        bp.setRight(toolBar);
        GridPane gp = new GridPane();
        gp.addRow(0, mFilterTextField, toolBar);
        GridPane.setHgrow(mFilterTextField, Priority.ALWAYS);
//
//        GridPane.setFillWidth(mFilterTextField, true);
//        mFilterTextField.setMaxWidth(Double.MAX_VALUE);
//        mFilterTextField.setPrefWidth(Double.MAX_VALUE);

//HBox hbox = new HBox(mFilterTextField, toolBar);
//        HBox.setHgrow(mFilterTextField, Priority.ALWAYS);
//        gp.setBackground(FxHelper.createBackground(Color.CORAL));
//        toolBar.setBackground(FxHelper.createBackground(Color.ALICEBLUE));
//        mFilterTextField.prefWidthProperty().bind(this.prefWidthProperty());
//        hbox.prefWidthProperty().bind(this.prefWidthProperty());
        toolBar.setStyle("-fx-spacing: 0px;");
        toolBar.setPadding(Insets.EMPTY);
//        toolBar.prefWidth(getIconSizeToolBarInt() * 2);
//        toolBar.getItems().add(0, mFilterTextField);
        setPrefWidth(300);
        setTop(gp);
        setCenter(mTreeView);
    }
