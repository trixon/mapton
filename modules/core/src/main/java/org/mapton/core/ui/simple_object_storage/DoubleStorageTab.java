 ...
 * @param <T>
 */
public class DoubleStorageTab<T extends MSimpleObjectStorageDouble> extends BaseTab {

    private final Class<T> mClass;
    private final HashMap<Class, Spinner<Double>> mClassToSpinner = new HashMap<>();
    private final StringConverter<Double> mDoubleConverter = new StringConverter<Double>() {
    ...
    public DoubleStorageTab(Class<T> c, String title) {
        super(title);
        mClass = c;
    ...
    public void load(Object object) {
        Lookup.getDefault().lookupAll(mClass).forEach(simpleStorage -> {
            ...
            var value = mManager.getDouble(simpleStorage.getClass(), simpleStorage.getDefaultValue());
    ...
    public void save(Object object) {
        Lookup.getDefault().lookupAll(mClass).forEach(simpleStorage -> {
            mManager.putDouble(simpleStorage.getClass(), mClassToSpinner.get(simpleStorage.getClass()).getValue());
        });
    }
    ...
    private void populateItems() {
        FxHelper.runLater(() -> {
            ...
            for (T simpleStorage : simpleStorages) {
                ...
                var spinner = new Spinner<Double>(Double.MIN_VALUE, Double.MAX_VALUE, 0.0);
                spinner.setPrefWidth(FxHelper.getUIScaled(150.0));
                spinner.setEditable(true);
                spinner.getValueFactory().setConverter(mDoubleConverter);
                FxHelper.autoCommitSpinner(spinner);
                var gp = new GridPane();
                gp.addRow(1, label, spinner);
                GridPane.setHgrow(label, Priority.ALWAYS);
                if (StringUtils.isNotBlank(simpleStorage.getTooltipText())) {
                    var tooltip = new Tooltip(simpleStorage.getTooltipText());
