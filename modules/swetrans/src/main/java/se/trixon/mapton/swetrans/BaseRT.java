...
public abstract class BaseRT implements CooTransProvider {
    ...
    public String getLatitudeString(double latitude, double longitude) {
        return String.format("%.1f X", getPosition(latitude, longitude, mProjection).getLatitude());
    }
    ...
    public String getLongitudeString(double latitude, double longitude) {
        return String.format("%.1f Y", getPosition(latitude, longitude, mProjection).getLongitude());
    }
    ...
    private RT90Position getPosition(double latitude, double longitude, RT90Projection projection) {
        return new RT90Position(new WGS84Position(latitude, longitude), projection);
    }
}
