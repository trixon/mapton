...
/**
 * This class of static methods provides the interface to logging for World Wind components. Logging is performed via {@link java.util.logging}. The default logger name is <code>gov.nasa.worldwind</code>. The logger name is configurable via {@link gov.nasa.worldwind.Configuration}.
 *
 * @author tag
...
public class Logging {
    ...
    protected static final int MAX_MESSAGE_REPEAT = 10;//Configuration.getIntegerValue(AVKey.MAX_MESSAGE_REPEAT, 10);
    ...
    public static Logger logger() {
        try {
            ...
            String loggerName = "AVList";//Configuration.getStringValue(AVKey.LOGGER_NAME, Configuration.DEFAULT_LOGGER_NAME);
            return logger(loggerName);
        } catch (Exception e) {
            return logger("AVList");
//            return logger(Configuration.DEFAULT_LOGGER_NAME);
        }
    }

    /**
     * Returns a specific logger. Does not access {@link gov.nasa.worldwind.Configuration} to determine the configured World Wind logger.
     * <p/>
     * This is needed by {@link gov.nasa.worldwind.Configuration} to avoid calls back into itself when its singleton instance is not yet instantiated.
     *
    ...
    /**
     * Retrieves a message from the World Wind message resource bundle formatted with a single argument. The argument is inserted into the message via {@link java.text.MessageFormat}.
     *
     * @param property the property identifying which message to retrieve.
    ...
    /**
     * Retrieves a message from the World Wind message resource bundle formatted with specified arguments. The arguments are inserted into the message via {@link java.text.MessageFormat}.
     *
     * @param property the property identifying which message to retrieve.
    ...
    /**
     * Indicates the maximum number of times the same log message should be repeated when generated in the same context, such as within a loop over renderables when operations in the loop consistently fail.
     *
