...
import org.mapton.butterfly_structural.load.LoadChartSOSB;
...
public class LoadChartOverlay extends BChartOverlay {

    public static final Color COLOR = Color.decode("#A62A2A");
    public static final int MAX_COUNT = 3;
    public static final int MAX_DISTANCE = 10;
    private final NumberAxis mAxis = new NumberAxis(SDict.LOAD_CELLS.toString());
    private final int mIndex = 107;

    public LoadChartOverlay() {
    ...
    public synchronized void plot(XYPlot plot, BBasePoint p, LocalDate aStartDate) {
        if (!mObjectStorageManager.getBoolean(LoadChartSOSB.class, LoadChartSOSB.DEFAULT_VALUE)) {
        ...
        var points = ButterflyHelper.getLimitedPoints(p, p.getButterfly().structural().getLoadPoints(), MAX_DISTANCE, MAX_COUNT, startDate);
