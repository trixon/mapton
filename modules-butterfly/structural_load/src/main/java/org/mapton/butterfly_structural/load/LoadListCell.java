 ...
 */
class LoadListCell extends ListCell<BStructuralLoadCellPoint> {
    ...
    @Override
    protected void updateItem(BStructuralLoadCellPoint p, boolean empty) {
    ...
    private void addContent(BStructuralLoadCellPoint p) {
        setText(null);
...
//        var nextDate = p.ext().getObservationRawNextDate();
    ...
    private class AlarmIndicator extends HBox {
        ...
        public void update(BStructuralLoadCellPoint p) {
            mResultantShape.setFill(LoadHelper.getAlarmColorHeightFx(p));
        }
