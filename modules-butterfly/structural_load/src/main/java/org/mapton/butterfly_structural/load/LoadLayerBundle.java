...
public class LoadLayerBundle extends BfLayerBundle {
    ...
    private final LoadLayerOptionsView mLayerOptionsView;
    private final LoadManager mManager = LoadManager.getInstance();

    public LoadLayerBundle() {
        init();
        initRepaint();
        mLayerOptionsView = new LoadLayerOptionsView(this);
    ...
    private void init() {
        initCommons(Bundle.CTL_LoadAction(), SDict.STRUCTURAL.toString(), "LoadTopComponent");
    ...
    private PointPlacemark plotLabel(BStructuralLoadCellPoint p, LoadLabelBy labelBy, Position position) {
        if (labelBy == LoadLabelBy.NONE) {
            return null;
        } else {
    ...
    private ArrayList<AVListImpl> plotSymbol(BStructuralLoadCellPoint p, Position position, PointPlacemark labelPlacemark) {
        ...
        var attrs = mAttributeManager.getAlarmInteriorAttributes(LoadHelper.getAlarmLevel(p));
