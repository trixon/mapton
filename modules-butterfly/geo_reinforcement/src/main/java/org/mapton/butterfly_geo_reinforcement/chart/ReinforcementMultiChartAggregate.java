 ...
 */
public class ReinforcementMultiChartAggregate {

    private final JTabbedPane mTabbedPane;

    public ReinforcementMultiChartAggregate() {
        mTabbedPane = new JTabbedPane(JTabbedPane.BOTTOM);
    }

    public synchronized Callable<JTabbedPane> build(BGeoReinforcementPoint p) {
        if (p == null) {
            return null;
        }
        ...
        var callable = (Callable<JTabbedPane>) () -> {
            synchronized (mTabbedPane) {
                ...
                Lookup.getDefault().lookupAll(BMultiChartPart.class).stream()
                        .filter(c -> StringUtils.equalsIgnoreCase(c.getCategory(), BGeoReinforcementPoint.class.getName()))
                        .sorted(Comparator.comparing(BMultiChartPart::getName))
                        .forEachOrdered(multiChartComponent -> {
                            try {
                                var chartBuilder = new ReinforcementMultiChartBuilder(
