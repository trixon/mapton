...
public class RoiManager extends BaseManager<BRoi> {
    ...
    @Override
    public Object getObjectProperties(BRoi selectedObject) {
    ...
    public void load(Butterfly butterfly) {
        try {
            initAllItems(butterfly.getRois());
...
//                item.ext().setDateFirst(item.getDateLatest());
    ...
    @Override
    protected void load(ArrayList<BRoi> items) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    @ServiceProvider(service = MDisruptorProvider.class)
    public static class PoiDisruptorProvider implements MDisruptorProvider {
    ...
    private static class Holder {
        ...
        private static final RoiManager INSTANCE = new RoiManager();
    }
}
