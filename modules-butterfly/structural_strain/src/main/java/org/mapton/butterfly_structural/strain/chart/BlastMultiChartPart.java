...
public abstract class BlastMultiChartPart extends BMultiChartPart {
    ...
    private final Predicate<BStructuralStrainGaugePoint> mPredicate;

    public BlastMultiChartPart(Predicate<BStructuralStrainGaugePoint> predicate) {
    ...
    public ArrayList<BStructuralStrainGaugePoint> getPoints(MLatLon latLon, LocalDate firstDate, LocalDate date, LocalDate lastDate) {
        ...
        for (var p : pointList) {
            var observations = p.ext().getObservationsTimeFiltered().stream()
                    ...
                    .map(o -> {
                        var oo = new BStructuralStrainGaugePointObservation();
