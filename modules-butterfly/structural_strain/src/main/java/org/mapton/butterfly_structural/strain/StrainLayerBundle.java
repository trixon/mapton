...
import org.mapton.butterfly_structural.strain.graphics.GraphicRenderer;
...
public class StrainLayerBundle extends BfLayerBundle {
    ...
    private final StrainLayerOptionsView mLayerOptionsView;
    private final StrainManager mManager = StrainManager.getInstance();

    public StrainLayerBundle() {
        init();
        initRepaint();
        mLayerOptionsView = new StrainLayerOptionsView(this);
    ...
    private void init() {
        initCommons(Bundle.CTL_StrainAction(), SDict.STRUCTURAL.toString(), "StrainTopComponent");
    ...
    private PointPlacemark plotPin(BStructuralStrainGaugePoint p, Position position, PointPlacemark labelPlacemark) {
        ...
        attrs = PinPaddle.W_SQUARE.applyToCopy(attrs);
