 ...
 */
public class StrainOptionsView extends BOptionsView {

    private final SessionComboBox<StrainColorBy> mColorScb = new SessionComboBox<>();
    private final StrainOptions mOptions = StrainOptions.getInstance();
    ...
    public StrainOptionsView(StrainLayerBundle layerBundle) {
        super(layerBundle, Bundle.CTL_StrainAction(), StrainOptions.getInstance(), "strain");
        createUI();
    ...
    private void createUI() {
        ...
        LabelBy.populateMenuButton(mLabelMenuButton, mOptions.labelByOperationProperty(), StrainLabelBy.values());
        mLabelMenuButton.setText(mOptions.getLabelFromId(StrainLabelBy.class, mOptions.labelByProperty().get().name(), StrainOptions.DEFAULT_LABEL_BY));
