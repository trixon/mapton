...
public class MultiChartAggregate {
    ...
    public synchronized Callable<JTabbedPane> build(BStructuralStrainGaugePoint p) {
        if (p == null) {
            return null;
        }
        ...
        var callable = (Callable<JTabbedPane>) () -> {
            synchronized (mTabbedPane) {
                ...
                Lookup.getDefault().lookupAll(BMultiChartPart.class).stream()
                        .filter(c -> Strings.CI.equals(c.getCategory(), BStructuralStrainGaugePoint.class.getName()))
