...
public class ChartBuilderAvgSplit extends BChartSplit {
    ...
    public ChartBuilderAvgSplit() {
        mLatestChartBuilder = new ChartBuilderDelta(true, 28);
        mCompleteChartBuilder = new ChartBuilderDelta(true, null);
    }

    public synchronized Callable<JPanel> build(BStructuralStrainGaugePoint p) {
        return build(p, true);
    }

    public synchronized Callable<JPanel> build(BStructuralStrainGaugePoint p, boolean standardLayout) {
        if (p == null) {
            return null;
        }
