...
public class PreDrillLayerBundle extends BfLayerBundle {
    ...
    private final PreDrillOptionsView mOptionsView;
    private final PreDrillOptions mOptions = PreDrillOptions.getInstance();

    public PreDrillLayerBundle() {
        init();
        initRepaint();
        mOptionsView = new PreDrillOptionsView(this);
    ...
    private PointPlacemark plotLabel(BGeoPreDrillPoint p, PreDrillLabelBy labelBy, Position position) {
        if (labelBy == PreDrillLabelBy.NONE) {
            return null;
        } else {
