 ...
 */
public class PreDrillMultiChartAggregate {

    private final JTabbedPane mTabbedPane;

    public PreDrillMultiChartAggregate() {
        mTabbedPane = new JTabbedPane(JTabbedPane.BOTTOM);
    }

    public synchronized Callable<JTabbedPane> build(BGeoPreDrillPoint p) {
        if (p == null) {
            return null;
        }
        ...
        var callable = (Callable<JTabbedPane>) () -> {
            synchronized (mTabbedPane) {
                ...
                Lookup.getDefault().lookupAll(BMultiChartPart.class).stream()
                        .filter(c -> StringUtils.equalsIgnoreCase(c.getCategory(), BGeoPreDrillPoint.class.getName()))
                        .sorted(Comparator.comparing(BMultiChartPart::getName))
                        .forEachOrdered(multiChartComponent -> {
                            try {
                                var chartBuilder = new PreDrillMultiChartBuilder(
