...
public class ChartBuilderDelta extends ChartBuilderBase {
    ...
    public void updateDataset(BHydroWaterLevelPoint p) {
        ...
        if (p.getDimension() != BDimension._2d) {
            delta1d = plot(p, mTimeSeries1d, Color.RED, (BHydroWaterLevelPointObservation o) -> o.getGroundwaterLevel());
        }
...
//            setRange(1.05, p.ext().getAlarm(BComponent.PLANE), p.ext().getAlarm(BComponent.HEIGHT));
            ...
            if (p.getDimension() != BDimension._2d) {
                sb.append("Δ %+.1f".formatted(delta1d));
            }
    ...
    private double plot(BHydroWaterLevelPoint p, TimeSeries timeSeries, Color color, Function<BHydroWaterLevelPointObservation, Double> function) {
        var plot = (XYPlot) mChart.getPlot();
...
//                renderer.setDefaultShapesVisible(false);
    ...
    private void plotMarkers(BHydroWaterLevelPoint p) {
        var plot = (XYPlot) mChart.getPlot();
