...
public class ChartBuilderDelta extends ChartBuilderBase {
    ...
    public void updateDataset(BHydroGroundwaterPoint p) {
        ...
        if (p.getDimension() != BDimension._2d) {
            delta1d = plot(p, mTimeSeries1d, Color.RED, (BHydroGroundwaterPointObservation o) -> o.getGroundwaterLevel());
        }
...
//            setRange(1.05, p.ext().getAlarm(BComponent.PLANE), p.ext().getAlarm(BComponent.HEIGHT));
    ...
    private double plot(BHydroGroundwaterPoint p, TimeSeries timeSeries, Color color, Function<BHydroGroundwaterPointObservation, Double> function) {
        var plot = (XYPlot) mChart.getPlot();
...
//                renderer.setDefaultShapesVisible(false);
    ...
    private void plotMarkers(BHydroGroundwaterPoint p) {
        var plot = (XYPlot) mChart.getPlot();
