 ...
 */
package org.mapton.butterfly_hydro.groundwater.chart.overlay;
...
public class GroundwaterChartOverlay extends BChartOverlay {
    ...
    public GroundwaterChartOverlay() {
        super(SDict.GROUNDWATER.toString());
    }
    ...
    public synchronized void plot(XYPlot plot, BBasePoint p, LocalDate aStartDate) {
        ...
        if (mObjectStorageManager.getBoolean(GroundwaterChartSOSB.class, GroundwaterChartSOSB.DEFAULT_VALUE)) {
            ...
            var points = ButterflyHelper.getLimitedPoints(p, p.getButterfly().hydro().getGroundwaterPoints(), true, MAX_DISTANCE, MAX_COUNT, startDate);
            if (!points.isEmpty()) {
