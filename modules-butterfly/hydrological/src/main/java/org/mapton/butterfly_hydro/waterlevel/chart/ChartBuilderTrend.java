...
public class ChartBuilderTrend extends ChartBuilderBase {
    ...
    private final Function<BHydroWaterLevelPointObservation, Double> mFunction;
    private final TimeSeries mTimeSeries;

    public ChartBuilderTrend(BDimension dimension, Function<BHydroWaterLevelPointObservation, Double> function) {
        mDimension = dimension;
        mFunction = function;
        mTimeSeries = new TimeSeries("Nivå");
        initChart("m", "0.0");
    }

    @Override
    public void updateDataset(BHydroWaterLevelPoint p) {
        mTimeSeries.clear();
...
//        setRange(1.05, p.ext().getAlarm(BComponent.PLANE), p.ext().getAlarm(BComponent.HEIGHT));
    ...
    private void plot(BHydroWaterLevelPoint p, String title, LocalDateTime startDate, LocalDateTime limitDate, Color color, int index, Integer percentile) {
        ...
        trend = TrendHelper.createTrend(p, true, startDate, endDate, (BXyzPointObservation o) -> 0d);
