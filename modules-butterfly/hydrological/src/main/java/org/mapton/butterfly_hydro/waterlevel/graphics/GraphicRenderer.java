...
import org.mapton.butterfly_hydro.waterlevel.WaterLevelAttributeManager;
import org.mapton.butterfly_hydro.waterlevel.WaterLevelManager;
...
public class GraphicRenderer extends GraphicRendererBase {
    ...
    @Override
    public void plot(BHydroWaterLevelPoint p, Position position, ArrayList<AVListImpl> mapObjects) {
    ...
    private Double getMedian(BHydroWaterLevelPoint p) {
        ...
        var waterLevels = list.stream()
                ...
                .mapToDouble(BHydroWaterLevelPointObservation::getGroundwaterLevel)
                .sorted();
    ...
    private void plotGroundwaterGraph(BHydroWaterLevelPoint p, Position position) {
        var offset = 20;
...
//        path.setShowPositions(true);
        ...
        var leftClickRunnable = (Runnable) () -> {
            Mapton.getGlobalState().put(BHydroWaterLevelPoint.class.getName() + "select", p);
        };
    ...
    private void plotTrace(BHydroWaterLevelPoint p, Position position, List<BHydroWaterLevelPointObservation> list, GraphicItem rendererItem) {
        ...
        var minObservation = list.stream()
                .filter(o -> o.getGroundwaterLevel() != null)
                .min(Comparator.comparing(BHydroWaterLevelPointObservation::getGroundwaterLevel))
                .orElse(null);
...
//                attrs.setInteriorMaterial(new Material(lightColor));
    ...
    private void plotTrace(BHydroWaterLevelPoint p, Position position, GraphicItem rendererItem, int months) {
