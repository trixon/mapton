...
public abstract class MultiChartPart extends BMultiChartPart {
    ...
    public static final double LIMIT_DISTANCE = 60.0;
...
//    public final double CUT_OFF_LIMIT = 0.002;
    ...
    public String getCategory() {
        return BHydroWaterLevelPoint.class.getName();
    }
    ...
    public BaseManager getManager() {
        return WaterLevelManager.getInstance();
    }

    @Override
    public ArrayList<BHydroWaterLevelPoint> getPoints(MLatLon latLon, LocalDate firstDate, LocalDate date, LocalDate lastDate) {
        var pointList = WaterLevelManager.getInstance().getTimeFilteredItems().stream()
                .filter(p -> {
                    switch (mDimension) {
                        case _1d -> {
                ...
                .filter(p -> {
                    return latLon.distance(BCoordinatrix.toLatLon(p)) <= LIMIT_DISTANCE;
                })
                .collect(Collectors.toCollection(ArrayList::new));

        var pointsToExclude = new ArrayList<BHydroWaterLevelPoint>();
        ...
        for (var p : pointList) {
            var observations = p.ext().getObservationsTimeFiltered().stream()
                    ...
                    .map(o -> {
                        var oo = new BHydroWaterLevelPointObservation();
...
//                            var accuZ = MathHelper.convertDoubleToDouble(o.ext().getAccuZ());
                            var valueZ = o.getGroundwaterLevel() - observations.getFirst().getGroundwaterLevel();
