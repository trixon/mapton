 ...
 */
package org.mapton.butterfly_composite.chart;
...
public abstract class MultiChartPart extends BMultiChartPart {
    ...
    public static final double LIMIT_DISTANCE_COMPOSITE = 40.0;
...
//    public final double CUT_OFF_LIMIT = 0.002;
    ...
    public BaseManager getManager() {
        return CompositeManager.getInstance();
    }
    ...
    public ArrayList<BCompositePoint> getPoints(MLatLon latLon, LocalDate firstDate, LocalDate date, LocalDate lastDate) {
        var pointList = CompositeManager.getInstance().getTimeFilteredItems().stream()
                ...
                .filter(p -> {
                    return latLon.distance(BCoordinatrix.toLatLon(p)) <= LIMIT_DISTANCE_COMPOSITE;
                })
