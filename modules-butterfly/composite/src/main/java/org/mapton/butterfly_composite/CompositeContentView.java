 ...
 */
public class CompositeContentView extends BContentView {

    private final CompositeManager mManager = CompositeManager.getInstance();

    public CompositeContentView(BContentOptions contentOptions) {
        super(contentOptions, new CompositeFilter());
        mFilterPopOver = new CompositeFilterPopOver(mFilter);
        mLayerOptions = CompositeLayerOptions.getInstance();

        mPresetPopOver = new MPresetPopOver(mFilterPopOver, MPresetPopOver.PARENT_NODE_FILTER, "composite");
