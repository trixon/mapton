...
public class GraphicRenderer extends BaseGraphicRenderer<GraphicItem, BRockEarthquake> {
    ...
    public void plot(BRockEarthquake quake, Position position, ArrayList<AVListImpl> mapObjects) {
        ...
        if (mCheckModel.isChecked(GraphicItem.BALLS)) {
            var timeSpan = ChronoUnit.MINUTES.between(quake.getDateLatest(), LocalDateTime.now());
        ...
        if (mCheckModel.isChecked(GraphicItem.BALLS_Z) && quake.getZeroZ() != null) {
            var altitude = quake.getZeroZ();
            var startPosition = WWHelper.positionFromPosition(position, 0.0);
            var endPosition = WWHelper.positionFromPosition(position, altitude);
            var radius = 1.2;
            var endEllipsoid = new Ellipsoid(endPosition, radius, radius, radius);
        ...
        if (mCheckModel.isChecked(GraphicItem.RECENT)) {
            var age = quake.ext().getMeasurementAge(ChronoUnit.DAYS);
            var maxAge = 30.0;
