 ...
 */
class TiltContentListCell extends BContentListCell<BStructuralTiltPoint> {
    ...
    public TiltContentListCell() {
        createUI();
    }

    @Override
    protected void addContent(BStructuralTiltPoint p) {
        setText(null);
        setGraphic(mVBox);
        loadTooltip(p);
...
//        var nextDate = p.ext().getObservationRawNextDate();
    ...
    private class AlarmIndicator extends BAlarmIndicator<BStructuralTiltPoint> {
        ...
        public void update(BStructuralTiltPoint p) {
            m1dShape.setFill(TiltHelper.getAlarmColorHeightFx(p));
        }
    }
}
