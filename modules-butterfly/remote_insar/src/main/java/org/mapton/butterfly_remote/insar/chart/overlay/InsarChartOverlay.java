 ...
 */
package org.mapton.butterfly_remote.insar.chart.overlay;
...
public class InsarChartOverlay extends BChartOverlay {

    public static final Color COLOR = Color.decode("#FFFF00");
    public static final int MAX_COUNT = 5;
    public static final int MAX_DISTANCE = 20;

    public InsarChartOverlay() {
        super("InSAR");
    }
    ...
    public synchronized void plot(XYPlot plot, BBasePoint p, LocalDate aStartDate) {
        ...
        if (mObjectStorageManager.getBoolean(InsarChartSOSB.class, InsarChartSOSB.DEFAULT_VALUE)) {
            var startDate = aStartDate == null ? LocalDate.now() : aStartDate;
            var points = ButterflyHelper.getLimitedPoints(p, p.getButterfly().remote().getInsarPoints(), true, MAX_DISTANCE, MAX_COUNT, startDate);
            if (!points.isEmpty()) {
                ...
                for (int i = 0; i < points.size(); i++) {
                    ...
                    for (var o : point.ext().getObservationsTimeFiltered()) {
                        if (o.getDate().isAfter(startDate.atStartOfDay())) {
                            var delta = o.ext().getDeltaZ() * InsarHelper.getScaleFactor(point);
