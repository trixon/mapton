...
public enum InsarLabelBy implements LabelBy.Operations {
    ...
    VALUE_DELTA_ZERO_Z(LabelBy.CAT_VALUE, "ΔZ", p -> {
        return p.ext().deltaZero().getDelta1(1, 1000, true);
    }),
    VALUE_BY_COLOR(LabelBy.CAT_VALUE, "Efter färg", p -> {
        return InsarAttributeManager.getInstance().getValueByColorBy(p);
    }),
    VALUE_VELOCITY(LabelBy.CAT_VALUE, "Hastighet", p -> {
        return "%+.1f".formatted(p.getVelocity());
    }),
    VALUE_VELOCITY_3M(LabelBy.CAT_VALUE, "Hastighet 3m", p -> {
        return "%+.1f".formatted(p.getVelocity3m());
    }),
    VALUE_VELOCITY_6M(LabelBy.CAT_VALUE, "Hastighet 6m", p -> {
        return "%+.1f".formatted(p.getVelocity6m());
    }),
    VALUE_ACCELERATION(LabelBy.CAT_VALUE, "Acceleration", p -> {
        return "%+.1f".formatted(p.getAcceleration());
    });
