...
public class UserLayerBundle extends BfLayerBundle {

    private final UserAttributeManager mAttributeManager = UserAttributeManager.getInstance();
    ...
    public UserLayerBundle() {
        init();
        initRepaint();
        mLayerOptionsView = new UserLayerOptionsView(this);
    ...
    private void init() {
        initCommons(Bundle.CTL_UserAction(), Dict.MISCELLANEOUS.toString(), "UserTopComponent");
    ...
    private PointPlacemark plotLabel(BXyzPoint p, UserLabelBy labelBy, Position position) {
        if (labelBy == UserLabelBy.NONE) {
            return null;
        } else {
