...
public class TunnelvattenLayerBundle extends BfLayerBundle {
    ...
    private void init() {
        mLayer.setName(Bundle.CTL_TunnelvattenAction());
        setCategory(mLayer, "TMO");
        setName(Bundle.CTL_TunnelvattenAction());
        attachTopComponentToLayer("TunnelvattenTopComponent", mLayer);
...
//                    mComponentRenderer.plot(p, position, mapObjects);
    ...
    private PointPlacemark plotLabel(BTunnelvatten p, TunnelvattenLabelBy labelBy, Position position) {
        if (labelBy == TunnelvattenLabelBy.NONE) {
            return null;
        }
