 ...
 */
public class TunnelvattenFilterPopOver extends BaseFilterPopOver {

    private final TunnelvattenFilter mFilter;
    ...
    public TunnelvattenFilterPopOver(TunnelvattenFilter filter) {
        mFilter = filter;
        createUI();
        initListeners();
        initSession();
    ...
    public void load(Butterfly butterfly) {
        var tunnelvatten = butterfly.tmo().getTunnelvatten();

        var groupCheckModel = mGroupCheckComboBox.getCheckModel();
        var checkedGroup = groupCheckModel.getCheckedItems();
        var allGroupss = new TreeSet<>(tunnelvatten.stream().map(o -> o.getGroup()).collect(Collectors.toSet()));
    ...
    private void initSession() {
        ...
        sessionManager.register("filter.tunnelvatten.freeText", mFilter.freeTextProperty());
        sessionManager.register("filter.tunnelvatten.checkedGroup", mGroupCheckModelSession.checkedStringProperty());
