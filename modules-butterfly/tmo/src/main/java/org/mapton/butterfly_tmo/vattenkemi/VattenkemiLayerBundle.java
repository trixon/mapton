...
public class VattenkemiLayerBundle extends BfLayerBundle {
    ...
    private void init() {
        mLayer.setName(Bundle.CTL_VattenkemiAction());
        setCategory(mLayer, "TMO");
        setName(Bundle.CTL_VattenkemiAction());
        attachTopComponentToLayer("VattenkemiTopComponent", mLayer);
...
//        mGroundConnectorLayer.setMaxActiveAltitude(1000);
    ...
    private void initListeners() {
        mManager.getTimeFilteredItems().addListener((ListChangeListener.Change<? extends BVattenkemi> c) -> {
            repaint();
        });
