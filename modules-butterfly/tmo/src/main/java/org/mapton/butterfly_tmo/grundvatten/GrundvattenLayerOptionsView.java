...
import org.mapton.butterfly_tmo.grundvatten.graphics.GraphicItem;
 ...
 */
public class GrundvattenLayerOptionsView extends BLayerOptionsView {

    private final SessionComboBox<GrundvattenColorBy> mColorScb = new SessionComboBox<>();
    ...
    private final SessionComboBox<GrundvattenPointBy> mPointScb = new SessionComboBox<>();

    public GrundvattenLayerOptionsView(GrundvattenLayerBundle layerBundle) {
        super(layerBundle, Bundle.CTL_GrundvattenAction(), GrundvattenLayerOptions.getInstance(), "grundvatten");
        setDefaultId(GrundvattenLayerOptions.DEFAULT_LABEL_BY);
    ...
    private void createUI() {
        mPointScb.getItems().setAll(GrundvattenPointBy.values());
        mColorScb.getItems().setAll(GrundvattenColorBy.values());
...
//        activateAnnotation();
    ...
    private void initSession() {
        ...
        restoreLabelFromId(GrundvattenLabelBy.class, mLayerOptions.labelByProperty().get().name(), GrundvattenLayerOptions.DEFAULT_LABEL_BY);
        labelByProperty().addListener((p, o, n) -> {
            for (var labelBy : GrundvattenLabelBy.values()) {
