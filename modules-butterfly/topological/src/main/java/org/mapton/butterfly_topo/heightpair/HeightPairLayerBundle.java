...
import org.mapton.butterfly_topo.grade.horizontal.graphics.GraphicRenderer;
...
public class HeightPairLayerBundle extends TopoBaseLayerBundle {

    private static final int PLOT_LIMIT = 10000;
    private final HeightPairAttributeManager mAttributeManager = HeightPairAttributeManager.getInstance();
    ...
    private final HeightPairOptionsView mOptionsView;

    public HeightPairLayerBundle() {
        init();
        initRepaint();
        mOptionsView = new HeightPairOptionsView(this);
    ...
    private void init() {
        initCommons(Bundle.CTL_HeightPairAction(), SDict.TOPOGRAPHY.toString(), "HeightPairTopComponent");
...
//                                mGraphicRenderer.plot(p, position, mapObjects);
    ...
    private PointPlacemark plotLabel(BTopoHeightPair p, HeightPairLabelBy labelBy, Position position) {
        if (labelBy == null || labelBy == HeightPairLabelBy.NONE) {
            return null;
        } else {
    ...
    private PointPlacemark plotPin(BTopoHeightPair p, Position position, PointPlacemark labelPlacemark) {
