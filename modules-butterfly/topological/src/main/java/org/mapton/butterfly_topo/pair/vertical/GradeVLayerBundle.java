...
public class GradeVLayerBundle extends TopoBaseLayerBundle {
    ...
    private Pair3OptionsView mOptionsView;
    ...
    public Node getOptionsView() {
        if (mOptionsView == null) {
            mOptionsView = (Pair3OptionsView) TabOptionsViewProvider.getInstance(Pair3OptionsView.class);
            if (mOptionsView != null) {
...
//            label = mOptionsView.getLabelBy().getLabel(p);
    ...
    private void init() {
        mLayer.setName(mBundle.getString("tilt_v"));
        setCategory(mLayer, SDict.TOPOGRAPHY.toString());
        setName(mBundle.getString("tilt_v"));
