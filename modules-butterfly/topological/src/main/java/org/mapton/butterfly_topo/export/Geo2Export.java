...
public class Geo2Export extends BaseExportProvider {
    ...
    public String getName() {
        return SimpleDialog.getExtensionFilters().get("geo").getDescription() + " senaste mätning";
    }
    ...
    private void setCoordinate(MCooTrans targetCT, GeoPoint point, BTopoControlPoint p) {
        ...
        var last = p.ext().getObservationFilteredLast();
        if (sourceCT == targetCT) {
            point.setX(last.getMeasuredY());
            point.setY(last.getMeasuredX());
            point.setZ(last.getMeasuredZ());
        } else {
            sourceCT.toWgs84(last.getMeasuredY(), last.getMeasuredX());
        }
    }
