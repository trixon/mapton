...
public class DistanceLayerBundle extends TopoBaseLayerBundle {
    ...
    private void init() {
        initCommons(Mapton.addWarning(mBundle.getString("grade_d"), 0), SDict.TOPOGRAPHY.toString() + "/Punktpar", "DistanceTopComponent");
...
//                    mapObjects.addAll(plotSymbol(p, position, labelPlacemark));
    ...
    private PointPlacemark plotLabel(BTopoGrade p, DistanceLabelBy labelBy, Position position) {
        if (labelBy == null || labelBy == DistanceLabelBy.NONE) {
            return null;
        } else {
