...
import org.mapton.butterfly_topo.grade.distancez.graphics.GraphicRenderer;
...
public class GradeDzLayerBundle extends TopoBaseLayerBundle {
    ...
    private final GradeDzManager mManager = GradeDzManager.getInstance();
    private final GradeDzOptionsView mOptionsView;
    private final GradeDzOptions mOptions = GradeDzOptions.getInstance();

    public GradeDzLayerBundle() {
        init();
        initRepaint();
        mOptionsView = new GradeDzOptionsView(this);
    ...
    private void init() {
        initCommons(Mapton.addWarning(Bundle.CTL_DistanceZAction(), 0), SDict.TOPOGRAPHY.toString(), "DistanceZTopComponent");
...
//                    mapObjects.addAll(plotSymbol(p, position, labelPlacemark));
    ...
    private PointPlacemark plotLabel(BTopoGrade p, GradeDzLabelBy labelBy, Position position) {
        if (labelBy == null || labelBy == GradeDzLabelBy.NONE) {
            return null;
        } else {
