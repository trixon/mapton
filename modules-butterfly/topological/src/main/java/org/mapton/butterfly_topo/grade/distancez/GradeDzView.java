 ...
 */
public class GradeDzView extends GradeView {

    public GradeDzView() {
        mManager = GradeDzManager.getInstance();
        ...
        config.setMaxDeltaR(GradeDzManager.MAX_RADIAL_DISTANCE);
        config.setAxis(BAxis.RESULTANT);

        mFilterPopOver = new GradeFilterPopOver(getClass(), mFilter, config);
        mPresetPopOver = new MPresetPopOver(mFilterPopOver, MPresetPopOver.PARENT_NODE_FILTER, "gradeDz");
        ...
        mListForm = new SingleListForm<>(mManager, mBundle.getString("grade_dz"));
        ...
        mListForm.getListView().setCellFactory(listView -> new GradeDzListCell());
