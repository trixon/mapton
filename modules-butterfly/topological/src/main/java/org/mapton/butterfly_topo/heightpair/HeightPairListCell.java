 ...
 */
class HeightPairListCell extends BListCell<BTopoHeightPair> {
    ...
    public HeightPairListCell() {
        createUI();
    }

    @Override
    protected void addContent(BTopoHeightPair p) {
        setText(null);
        setGraphic(mVBox);
...
//        mDesc3Label.setText("%s (%d)".formatted(p.getPeriod(), p.getCommonObservations().size()));
    ...
    private class AlarmIndicator extends BAlarmIndicator<BTopoHeightPair> {
