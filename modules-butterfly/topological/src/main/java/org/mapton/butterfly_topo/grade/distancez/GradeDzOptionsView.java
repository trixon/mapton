...
import org.mapton.butterfly_topo.grade.distancez.graphics.GraphicItem;
 ...
 */
public class GradeDzOptionsView extends BOptionsView {

    private final SessionComboBox<GradeDzColorBy> mColorScb = new SessionComboBox<>();
    private final SessionCheckComboBox<GraphicItem> mGraphicSccb = new SessionCheckComboBox<>();
    private final GradeDzOptions mOptions = GradeDzOptions.getInstance();
    private final SessionComboBox<GradePointBy> mPointScb = new SessionComboBox<>();

    public GradeDzOptionsView(GradeDzLayerBundle layerBundle) {
        super(layerBundle, NbBundle.getMessage(GradeManagerBase.class, "grade_d"), GradeDzOptions.getInstance(), "distance");
        createUI();
    ...
    private void createUI() {
        ...
        mColorScb.getItems().setAll(GradeDzColorBy.values());
        ...
        LabelBy.populateMenuButton(mLabelMenuButton, mOptions.labelByOperationProperty(), GradeDzLabelBy.values());
        mLabelMenuButton.setText(mOptions.getLabelFromId(GradeDzLabelBy.class, mOptions.labelByProperty().get().name(), GradeDzOptions.DEFAULT_LABEL_BY));
...
//        activateAnnotation();
    ...
    private void initListerners() {
        mOptions.labelByOperationProperty().addListener((p, o, n) -> {
            ...
            for (var labelBy : GradeDzLabelBy.values()) {
