...
import org.mapton.butterfly_meteo.graphics.GraphicRenderer;
...
public class MeteoLayerBundle extends BfLayerBundle {

    private final MeteoAttributeManager mAttributeManager = MeteoAttributeManager.getInstance();
    private final GraphicRenderer mGraphicRenderer;
    private final MeteoLayerOptions mLayerOptions = MeteoLayerOptions.getInstance();
    private final MeteoLayerOptionsView mLayerOptionsView;
    private final MeteoManager mManager = MeteoManager.getInstance();

    public MeteoLayerBundle() {
        init();
        initRepaint();
        mLayerOptionsView = new MeteoLayerOptionsView(this);
    ...
    private void init() {
        initCommons(SDict.METEOROLOGY.toString(), SDict.HYDRO_METEO.toString(), "MeteoTopComponent");

        mLayer.setMaxActiveAltitude(6000);
        mSurfaceLayer.setMaxActiveAltitude(6000);
//        mPinLayer.setMaxActiveAltitude(20000);
        mLabelLayer.setMaxActiveAltitude(500_000);
    ...
    private PointPlacemark plotLabel(BMeteoPoint p, MeteoLabelBy labelBy, Position position) {
        if (labelBy == MeteoLabelBy.NONE) {
            return null;
        } else {
    ...
    private PointPlacemark plotPin(Position position, PointPlacemark labelPlacemark) {
        var attrs = mAttributeManager.getPinAttributes(Color.YELLOW);
