...
//        final Function<BMeteoPointObservation, Double> func1d = (var o) -> o.getGroundwaterLevel();
//        mBuilderTrend1d = new ChartBuilderTrend(BDimension._1d, func1d);

        mTabbedPane = new JTabbedPane(JTabbedPane.BOTTOM);
    }

    public synchronized Callable<JTabbedPane> build(BMeteoPoint p) {
        if (p == null) {
            return null;
        }
        ...
        var callable = (Callable<JTabbedPane>) () -> {
            synchronized (mTabbedPane) {
                ...
                if (p.ext().getObservationsTimeFiltered().size() > 1) {
                    mTabbedPane.add("Temperatur", mBuilderDeltaSplit.build(p).call());
